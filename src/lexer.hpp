...
enum tokens {
	...
	//
	// Trebuie sterse astea in afara de token_ASSIGN_FLOAT si
	// token_ASSIGN_STRING
...
/// Functie auxiliara, verifica daca urmatorul caracter poate fi considerat
/// drept facand parte dintr-un alt token
bool checkNextCharacterDifferentToken(const std::string& code, int i) {
	...
	// cuvantul e urmat de whitespace, poate mai trebuie adaugate alte tipuri de
	// caractere whitespace
	if (nextCharacter == ' ' || nextCharacter == '\n' || nextCharacter == '\t')
		return true;

	// cuvantul e urmat de marcator sfarsit de linie (se poate scoate daca nu
	// mai mergem pe ideea asta)
	...
	// nu am gasit un caracter ce poate marca inceputul unui alt token,
	// presupunem ca este un caracter de genul ~`#:;',.? (nu stiu ce se
	// considera in cazul asta, o sa presupun ca pot face parte din cuvinte)
	return false;
}

/// Functie auxiliara, verifica daca code[start .. start + word.length()] ==
/// word si daca tockenul se incheie acolo
bool checkIfToken(const std::string& code, int start, const std::string& word) {
	int i;
	bool poate_fi = true;

	// verifica daca toate caracterele sunt egale
	for (i = 0; (i < (int)word.length()) && poate_fi &&
	            (start + i < (int)code.length());
	     ++i)
...
/**
Ia urmatorul token si pune-l in currentToken. Modifica variabilele globale 'p',
'character' si 'currentToken'. Probabil ar fi o idee buna sa nu mai folosim
variabile globale pentru ca pot aparea erori si in cazul ala pierzi starea
precedenta a variabilelor.
@return Returneaza tipul de token; Un token este un keyword / cuvant + (un
spatiu sau end on line)
*/
int getNextToken() {
	if (p < 0 or p >= (int)sourceCode.length()) {
		currentToken = "";
		throw "getNextToken called with invalid p = " + std::to_string(p) +
		    " when source code length = " + std::to_string(sourceCode.length());
	}
	...
	if (character == ' ' or character == '\t') {
		if (p == 0) {
			...
			//
			// Token-ul returnat va fi token_IDENTATION indiferent de numarul de
			// identari Numarul de identari este pus in currentToken
			...
			identation_cnt +=
			    tabs_cnt;  // tab-uri sunt echivalente cu identarea
			identation_cnt +=
			    spaces_cnt / 4 + (spaces_cnt % 4 and 1);  // formula pentru a
			                                              // transforma spatii
			                                              // in identare
	...
	if (isdigit(character)) {
		...
		return token_FLOAT;
	           ...
	           p < (int)sourceCode.length() -
	                   2) {  // Aici ar trebui adaugat de fapt un check pentru
		                     // siruri de
		// caractere care sa returneze eroare daca e inceput dar nu e terminat
		int start = p;
		currentToken += character;
		while (p <
		       (int)sourceCode.length() - 1) {  // Aici s-ar putea face mult mai
			                                    // simplu ca sa nu verifici la
			// sfarsit din nou
			character = sourceCode[++p];
			if (character == '\"') {
	...
	// Keywords (Input, Output, Structuri de decizie, Structuri repetitive)
	{
		...
		std::string keywords[cnt_keywords] = {
		    "citeste",     "scrie",    "daca",    "atunci", "altfel",
		    "altfel daca", "cat timp", "executa", "pentru", "pana cand"};
		tokens keyword_tok[cnt_keywords] = {
		    token_INPUT, token_OUTPUT,  token_IF,    token_THEN,
		    token_ELSE,  token_ELSE_IF, token_WHILE, token_EXECUTE,
		    token_FOR,   token_UNTIL};
	...
	if (character == '<') {  // Asta pare foarte gresit;
		// Codul "daca x<-y atunci" pare ca produce un token de atribuire dar
		// dupa cum am discutat nu asa va functiona
		if (sourceCode[p + 1] == '-') {
			currentToken = "<-";
			p += 2;
			return token_ASSIGN;
	...
	// Data types
	{
		...
		std::string keywords[cnt_keywords] = {"(numar natural)",
		                                      "(numere naturale)",
		                                      "(numar natural, nenul)",
		                                      ...
		                                      "(bool)",
		                                      "(logica)"};
		tokens keyword_tok[cnt_keywords / 2] = {
		    token_ASSIGN_NATURAL, token_ASSIGN_nonzeroNATURAL,
		    token_ASSIGN_INT,     token_ASSIGN_nonzeroINT,
		    token_ASSIGN_FLOAT,   token_ASSIGN_nonzeroFLOAT,
		    token_ASSIGN_STRING,  token_ASSIGN_BOOL};

...
void execute(statements* Statements, bool inTerminal) {
	for (statement* Statement : Statements->s) {
		if (Statement->outAst_p) {
			...
			if (inTerminal) {
				if (Statement->exprAst_p->op == numberType)
					std::cout << Statement->exprAst_p->number;
				else
					std::cout << Statement->exprAst_p->str;
				std::cout << '\n';
			}
		}
	}
	...
	if (argc == 0) {
		...
		while (getline(std::cin, sourceCode)) {
			...
			std::vector<std::pair<std::string, int>>
			    tokens{};  // every token and its type on this line
			...
			try {
				...
				switch (n) {
					case syntaxError:
						errors::syntax_error(filename, sourceCode2,
						                     line_number);
						break;
					case divisionByZero:
						errors::division_by_zero(filename, sourceCode2,
						                         line_number);
						break;
					case invalidFile:
					...
					case uninitialisedVar:
						errors::uninitialised_var(filename, sourceCode2,
						                          line_number);
						break;
				}
			}
		...
		try {
			/* Citeste din fisier, pune in variabila sourceCode (declarara in
			 * lexer.h) si interpreteaza */
			...
			if (sourceFile) {
				while (getline(sourceFile, sourceCode)) {
					...
					sourceCode += "$";
					while (character != '$' and
					       p < (int)sourceCode.length() - 1) {
						int x = getNextToken();

						std::pair<std::string, int> token(currentToken, x);

						// token_FORCE_QUIT este un token care este returnat
						// atunci cand pe linie, ultimul caracter este ' ', '\t'
						// Acesta trebuie ignorat
						if (x != token_FORCE_QUIT) {
							tokens.push_back(token);
						}
					...
					p = 0;
					character =
					    ' ';  // asta putea fi setat la orice in afara de $
				...
				// Timpul sa executam tree-ul pe care l-am creat (variabila
				// Statements este tot tree-ul)

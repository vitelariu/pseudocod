...
void checkFlags(exprAst* Tree, double result) {
	if (Tree->flags[exprAst::INT_FLAG]) {
		if (!Tree->flags[exprAst::INT_BUG_FIX_FLAG]) {
			if (result >= 0 or (result < 0 and result == (int)result)) {
				result = (int)result;
			} else {
				result = (int)result;
				result--;
...
// Ar fi bine sa se poata intelege ce face functia asta de cineva care nu a
// apucat sa lucreze la proiect foarte mult
...
void interpret(exprAst* Tree) {
	...
	switch (Tree->op) {
		case orType:
			...
			// Chestia asta poate scapa de sub control, asa ca o sa facem o mica
			// optimizare interpret(Tree->right); aux(Tree->right);
			...
			if (Tree->left->number)
				...
				aux(Tree->right);
				if (Tree->left->op == stringType or
				    Tree->right->op == stringType)
					throw syntaxError;
		...
		case greaterType:
			...
			if ((Tree->left->op == stringType) ^
			    (Tree->right->op == stringType)) {
				throw syntaxError;
			} else if (Tree->left->op == stringType and
			           Tree->right->op == stringType) {
				if (getString(Tree->left->str)
				        .compare(getString(Tree->right->str)) > 0) {
					checkFlags(Tree, 1);
				} else {
					checkFlags(Tree, 0);
				}
		...
		case plusType:
			...
			if (Tree->left->op == stringType and Tree->left->op == stringType) {
				Tree->op = stringType;
				Tree->str = "\"" + getStringRaw(Tree->left->str) +
				            getStringRaw(Tree->right->str) + "\"";
		...
		case minusType:
			...
			if (Tree->left->op == stringType or Tree->right->op == stringType)
				throw syntaxError;
		...
		case multiplyType:
			interpret(Tree->left);
			interpret(Tree->right);
			if (Tree->left->op == stringType and
			    Tree->right->op == numberType) {
				Tree->op = stringType;
				Tree->str = multiply_str(Tree->left->str, Tree->right->number);
				clean(Tree);
				break;
			} else if (Tree->right->op == stringType and
			           Tree->left->op == numberType) {
				Tree->op = stringType;
				Tree->str = multiply_str(Tree->right->str, Tree->left->number);
				clean(Tree);
				break;

			} else if (Tree->left->op == stringType or
			           Tree->right->op == stringType) {
				throw syntaxError;
			}
		...
		case moduloType:
			...
			if (Tree->left->op == stringType or Tree->right->op == stringType)
				throw syntaxError;
			if (Tree->left->number != (int)Tree->left->number or
			    Tree->right->number != (int)Tree->right->number) {
				throw syntaxError;
...
void interpretEntry(outAst* Tree) {
	for (int i{}; i < (int)Tree->list_of_exprTree.size(); i++) {
		...
		if (Tree->list_of_exprTree[i]->op == numberType) {
			std::cout << Tree->list_of_exprTree[i]->number;
		} else {

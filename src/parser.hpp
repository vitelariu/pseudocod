...
enum types {
	numberType,
	stringType,
	unknownType,  // Type that has a special use (it means that cand be either
	              // numberType or stringType)
	varType,
...
/** AST = Abstract syntax tree. E o clasa ce reprezinta un arbore binar in care
 fiecare nod poate fi evaluat la o singura valoare, cu proprietatea ca evaluarea
 radacinii da rezultatul evaluarii expresiei originale. Mai jos este definita
 structura nodurilor unui astfel de arbore (ar trebui redenumita clasa la
 exprAstNode)
*/
class exprAst {
   public:
	...
	// 2 -> plus (se va folosi pointerele de mai jos (exprAst *left/right)
	// pentru a reprezenta nod-urile) etc etc
	int op;
	double number;
	std::string str;
	variable var;
	...
	//
	// - flags[1]
	// daca este true, numarul, sau expresia in cazul -(1 + 2) etc, va fi
	// negativa
	//
	// - flags[2]
	...
	//
	// - flags[4]
	// rol in fixarea unui bug legat de convertirea unui nr real negativ in
	// intreg.
	//
	// - flags[5]
	...
	exprAst(int Type, exprAst* left, exprAst* right)
	    : op(Type),
	      number(0),
	      str(),
	      var(),
...
class parseExpr {
   private:
	...
	parseExpr(std::vector<std::pair<std::string, int>>&& tokens)
	    : tokens(std::move(tokens)) {}
	parseExpr(const std::vector<std::pair<std::string, int>>& tokens)
	    : tokens(tokens) {}
	...
	exprAst* exp() {
		if (token.second == token_FLOAT) {
			...
			if (Var.type == -1) {
				// Asta inseamna ca nici macar nu exista aceasta variabila si
				// este folosita intr-o expr;
			...
			// while-ul de mai jos e folsit pentru a fixa un bug cand un numar
			// negativ e inmultit cu putere: -(expr)^y = -1 * (expr)^y
			int index_copy = index - 1, np_cnt{};
			bool nextpow{};
			while (index_copy < (int)tokens.size()) {
				int searchtk = tokens[index_copy].second;
				if (searchtk == token_LEFT_PARENTH or
				    searchtk == token_LEFT_SQUARE) {
					np_cnt++;
				} else if (searchtk == token_RIGHT_PARENTH or
				           searchtk == token_RIGHT_SQUARE) {
					np_cnt--;
					index_copy++;
					continue;
				}
				...
				if (np_cnt == 0) {
					if (searchtk != token_FLOAT and
					    searchtk != token_IDENTIFIER) {
			...
			if (token.second != token_LEFT_PARENTH and
			    token.second != token_FLOAT) {
				throw syntaxError;
			} else {
				exprAst* nested = exp();
	...
	exprAst* factor() {
		...
		while (true) {
			...
			if (token.second == token_RIGHT_PARENTH or
			    token.second == token_RIGHT_SQUARE)
				op = true;
			if (!op) {
				throw syntaxError;
			}
			...
			if (token.second == token_POWER) {
				...
				if ((token.second == token_IDENTIFIER or
				     token.second == token_FLOAT or
				     token.second == token_LEFT_PARENTH or
				     token.second == token_LEFT_SQUARE or
				     token.second == token_MINUS or
				     token.second == token_PLUS or
				     token.second == token_NOT) and
				    result->op != stringType) {
					if (exprTree->op == numberType) {
						...
						exprTree_copy->right =
						    new exprAst(powerType, exprTree_copy->right, exp());
					}
	...
	exprAst* term() {
		...
		while (true) {
			...
			if (token.second != token_ASTERISK and
			    token.second != token_DIVISION and token.second != token_MODULO)
				break;
			...
			if (token.second == token_ASTERISK) {
				...
				if (token.second == token_IDENTIFIER or
				    ...
				    token.second == token_LEFT_PARENTH or
				    token.second == token_LEFT_SQUARE or
				    token.second == token_MINUS or token.second == token_PLUS or
				    token.second == token_NOT or token.second == token_STRING) {
	...
	exprAst* addend() {
		...
		while (true) {
			...
			if (token.second != token_PLUS and token.second != token_MINUS)
				break;
			...
			if (token.second == token_PLUS) {
				...
				} else if ((token.second == token_IDENTIFIER or
				            token.second == token_STRING or
				            token.second == token_LEFT_PARENTH) or
				           result->op == stringType) {
					exprTree = new exprAst(plusType, exprTree, term());
				} else {
	...
	exprAst* comp() {
		...
		while (true) {
			...
			if (token.second != token_GREATER and
			    token.second != token_GREATER_EQUAL and
			    token.second != token_SMALLER and
			    token.second != token_SMALLER_EQUAL and
			    token.second != token_EQUAL and token.second != token_NOT_EQUAL)
				break;
			...
			if (token.second == token_GREATER) {
				...
				if (token.second == token_IDENTIFIER or
				    ...
				    token.second == token_NOT) {
					exprTree =
					    new exprAst(greaterEqualType, exprTree, addend());
				} else {
					throw syntaxError;
				}
				...
				if (token.second == token_IDENTIFIER or
				    ...
				    token.second == token_NOT) {
					exprTree =
					    new exprAst(smallerEqualType, exprTree, addend());
	...
	exprAst* expr() {
		...
		expr : logic ((OR) logic)*
		logic : comp ((AND) comp)*
		comp :  addend ((GREATER | GREATER_EQUAL | SMALLER | SMALLER_EQUAL |
		EQUAL | NOT_EQUAL) addend)* addend : term ((PLUS | MINUS) term)* term :
		factor ((MUL | DIV | MOD) factor)* factor : exp ((POW) exp)* exp :
		INTEGER | LPAREN expr RPAREN | LSQUARE expr SQUARE | NOT_EQUAL exp
		*/
		exprAst* exprTree = nullptr;
		...
		while (true) {
			...
			if ((token.second == token_IDENTIFIER or
			     ...
			     token.second == token_LEFT_PARENTH or
			     token.second == token_LEFT_SQUARE or
			     token.second == token_PLUS or token.second == token_NOT) and
			    result->op != stringType) {
		...
		if (token.second != token_IDENTIFIER and token.second != token_FLOAT and
		    token.second != token_RIGHT_PARENTH and
		    token.second != token_STRING and
		    token.second != token_RIGHT_SQUARE and
		    token.second != token_FORCE_QUIT) {
			throw syntaxError;
		}
...
class outAst {
   public:
	...
	void add(exprAst* exprTree) { list_of_exprTree.push_back(exprTree); }
	~outAst() {
...
class parseOut {
   private:
	...
	parseOut(std::vector<std::pair<std::string, int>>&& tokens)
	    : tokens(std::move(tokens)) {}
	parseOut(const std::vector<std::pair<std::string, int>>& tokens)
	    : tokens(tokens) {}
...
class parseVar {
	...
	parseVar(std::vector<std::pair<std::string, int>>&& tokens)
	    : tokens(std::move(tokens)) {}
	parseVar(const std::vector<std::pair<std::string, int>>& tokens)
	    : tokens(tokens) {}
	...
	varNode* convert() {
		...
		// Nu am cum sa stiu daca expr va fi stringType sau numberType pana nu o
		// execut asa a folosesc unknwonType
...
class inAst {
   public:
	...
	void add(std::string name) { list_of_identifiers.push_back(name); }
};
...
class parseIn {
	...
	parseIn(std::vector<std::pair<std::string, int>>&& tokens)
	    : tokens(std::move(tokens)) {}
	parseIn(const std::vector<std::pair<std::string, int>>& tokens)
	    : tokens(tokens) {}
	...
	inAst* convert() {
		...
		getNextTokenFromVector();
		if (token.second == token_IDENTIFIER)
			Tree->add(token.first);
		else
			throw syntaxError;
		if (index < (int)tokens.size()) {
			getNextTokenFromVector();
		...
		for (std::string id : Tree->list_of_identifiers) {
			...
			// pot de fapt sa determin daca e stringType sau numberType dar nu
			// am nevoie oricum asa ca folosesc unkownType
			variable.type = unknownType;
...
class statements {
   public:
	...
	void add(statement* st) { s.push_back(st); }
};
